...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await db.from('races').select('id,status,started_at,ended_at,rooms(id,code,name,status,max_players),song_texts(id,title,artist,text_content)').eq('id', params.id).single()
